...
public class Hangman {
	...
	RandomWord randomWord = new RandomWord(); // randomWord.getWords(); 모든 단어 출력
	...
	public void run() {
		do {
			...
			getUserInput(); //철자 맞추기
			checkInput(); //맞는지 체크 (맞을 시 running=false)
		} while (running);
	}
	
	private void displayWord() { //체크
		System.out.println(randomWord); //선택된 단어 하나 출력
	}
	...
	private void getUserInput() {
		...
		//RandomWord 클래스에 전달( addGuess(문자) 메소드 만들기 )
